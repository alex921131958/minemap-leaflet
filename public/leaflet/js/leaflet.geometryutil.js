...
(function (factory) {
  ...
  L.GeometryUtil = L.extend(L.GeometryUtil || {}, {
    ...
    getPixelsPerMeter: function (map) {
      var centerLatLng = map.getCenter(); // get map center
      var pointC = map.latLngToContainerPoint(centerLatLng); // convert to containerpoint (pixels)
      var pointX = L.point(pointC.x + 100, pointC.y); // add 10 pixels to x

      // convert containerpoints to latlng's
      var latLngX = map.containerPointToLatLng(pointX);

      var meters = centerLatLng.distanceTo(latLngX) / 100; // calculate distance between c and x (latitude)

      return (1 / meters);
    },

    latLngToLayerPoint: function (map, latlng) {
    ...
    closest: function (map, layer, latlng, vertices) {
      ...
      latlngs = [];
      var tmplatlngs = layer.getLatLngs();

      if (Array.isArray(tmplatlngs[0])) {
        var _latlngs = [];
        for (var i = 0; i < tmplatlngs.length; i++) {
          var _tmp = tmplatlngs[i];

          for (var j = 0; j < _tmp.length; j++) {
            _latlngs.push({lat: _tmp[j].lat, lng: _tmp[j].lng});
          }
          latlngs.push(_latlngs);
        }
      } else {
        for (var i = 0; i < tmplatlngs.length; i++) {
          latlngs.push({lat: tmplatlngs[i].lat, lng: tmplatlngs[i].lng});
        }
      }
